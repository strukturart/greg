const eximport = (() => {
        ...
        setTimeout(function () { // convert
            ...
            data.forEach((e) => {
                ...
                for (let key in e) {
                    ...
                    if (key != "BEGIN" && key != "END" && key != "date" && key != "time_start" && key != "time_end" && key != "dateStart" && key != "dateEnd" && key != "notification" && key != "alarm" && key != "isSubscription" && key != "multidayevent" && key != "alarmTrigger") {
                        result += `${key}:${
                            e[key]
                        }` + "\r\n";
                    }
    ...
    let list_ics = function () { // search .ics files
    ...
    let parse_ics = function (data, callback, saveOnDevice, subscription) {
        ...
        vcalendar.getAllSubcomponents("vevent").forEach(function (index) {
            if (index.getFirstPropertyValue("dtstart") == "" && index.getFirstPropertyValue("summary") == "") 
                return false;
            ...
            if (index.getFirstPropertyValue("dtstart") != null) {
                ...
                dateStart = DTstart.getFullYear() + "-" + `0${
                    DTstart.getMonth() + 1
                }`.slice(-2) + "-" + `0${
                    DTstart.getDate()
                }`.slice(-2);
        ...
        if (saveOnDevice) {
            ...
            localforage.setItem("events", without_subscription).then(function (value) { // events = value;
            }).catch(function (err) {
                console.log(err);
            });
        }
    ...
    function loadICS(filename, callback) {
        ...
        finder.on("fileFound", function (file, fileinfo, storageName) { // file reader
            ...
            reader.onloadend = function (event) { // text to array
