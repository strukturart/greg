...
let event_check = function (date) {
    ...
    for (let t = 0; t < events.length; t++) { // multi day event
...
let showCalendar = function (month, year) {
    ...
    // creating all cells
    let date = 1;
    for (let i = 0; i < 5; i++) { // creates a table row
...
function renderHello(arr) {
    ...
    document.querySelectorAll("article").forEach(function (index) {
        ...
        let d = weekday[t.getDay()] + ", " + t.getFullYear() + " " + months[t.getMonth()] + " " + t.getDate();

        let m = weekday[n.getDay()] + ", " + n.getFullYear() + " " + months[n.getMonth()] + " " + n.getDate();
...
if ("b2g.alarmManager" in navigator) {
    navigator.serviceWorker.register("assets/js/service-worker.js").then((registration) => {
        registration.systemMessageManager.subscribe("alarm").then((rv) => {
...
let router = function (view) {
    ...
    if (status.view == "options") {
        ...
        document.querySelectorAll("button.dynamic").forEach(function (item) { // item.remove();
        });
        option_button_bar();
    }
...
let store_subscription = function () {
    if (helper.validate(document.getElementById("cal-subs-url").value) && document.getElementById("cal-subs-name").value != "") {
        subscriptions.push({url: document.getElementById("cal-subs-url").value, name: document.getElementById("cal-subs-name").value});
...
let delete_subscription = function () {
    ...
    localforage.setItem("subscriptions", updated_subscriptions).then(function (value) { // Do other things once the value has been saved.
...
document.addEventListener("DOMContentLoaded", function (e) {
    ...
    let add_alarm = function (date, message_text, id) { // KaiOs 3.0

        if ("b2g.alarmManager" in navigator) { // This is arbitrary data pass to the alarm
            var data = {
                foo: message_text,
                event_id: id
            };
        ...
        // KaiOs  2.xx
        if (navigator.mozAlarms) { // This is arbitrary data pass to the alarm
            var data = {
                foo: message_text,
                event_id: id
            };
    ...
    // with all events
    // to clean
    let remove_alarm = function (id) { // KaiOs  3.00
    ...
    let store_event = function () {
        ...
        let without_subscription = events.filter((events) => events.isSubscription === false)
    ...
    let update_event = function () {
        ...
        localforage.setItem("events", without_subscription).then(function (value) { // clean form
            renderHello(events);
    ...
    let edit_event = function () {
        events.forEach(function (index) {
            if (index.UID == status.selected_day_id) {
                ...
                document.getElementById("form-image").src = index.image;
    ...
    let delete_event = function () {
        ...
        localforage.setItem("events", without_subscription).then(function (value) { // Do other things once the value has been saved.
            console.log("saved: " + value);
            renderHello(events);
    ...
    function longpress_action(param) {
        switch (param.key) {
            ...
            case "ArrowLeft":
                if (status.view == "list-view") { // to do
                    delete_event();
    ...
    function shortpress_action(param) {
        switch (param.key) {
            ...
            case "Enter":
                ...
                if (status.view == "subscription") { // store subscription
                    store_subscription();
                    return true;
                }
