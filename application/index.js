...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function showCalendar(month, year) {
    //let month_year = months[month] + " " + year;

    helper.bottom_bar("view", "select", "options");
    ...
    for (let i = 0; i < 6; i++) {
      ...
      for (let j = 0; j < 7; j++) {
        if (i === 0 && j < firstDay) {
          ...
          let p = currentYear + "-" + (currentMonth + 1) + "-" + date;
  ...
  let nav = function (move) {
    console.log(console.log(JSON.stringify(status)));
